...
<<tableOne, results="asis", echo = FALSE>>=
	summ.table <- data.frame(Name=c("Presence points","Groups","Background points"),
...
model using all sites as input. Importance values (mean decrease in accuracy) are extracted from the randomForest{\protect\NoHyper\cite{liaw2002}\protect\endNoHyper} function. See Appendix 1 for variable descriptions.
\end{minipage}% 
...
\small
Table 3. Thresholds {\protect\NoHyper\cite{LiuEtAl2005, LiuEtAl2015}\protect\endNoHyper} calculated from the final model. The Value column reports the threshold; Groups indicates the percentage (number in brackets) of groups within which at least one point was predicted as suitable habitat; Pct indicates the percentage of presence reaches predicted having suitable habitat. Total numbers of presence reaches and contiguous presence reach groups used in the final model are reported in Table 1.
...
<<tableThree, results="asis", echo = FALSE>>=
  ...
  print(xtable(tbl, digits = 3, align = c("r","p{2in}","r","r","r","p{3.3in}")), 
              floating=FALSE, include.rownames=FALSE)
@
\end{center}
\end{minipage}
...
##plot(results_shape$geometry, col = NA, axes = F) 
# could use dissolved range here
# plot(st_transform(studyAreaExtent$geometry, st_crs(results_shape)), col = NA, border = NA, axes = TRUE)
...
# plot it
qtm(basetiles) + 
  tm_shape(results_shape, name="modeled suitability") + # stream linework #, simplify = 0.5
    tm_lines(col="prbblty", palette=clrs, labels=c("Low Habitat Suitability", rep(" ", nclr-2), "High Habitat Suitability"),
             title.col = "modeled suitability") +     
  if(exists("aquaPolys")) tm_shape(aquaPolys) + tm_fill(col="prbblty", palette=clrs, legend.show = FALSE) +     
...
\includegraphics[width=1.0\linewidth]{../../../../../NatureServeLogo}%png logo file at repository root
\end{minipage}%
\begin{minipage}[c]{0.75\linewidth}
Please cite this document and its associated SDM as: \\
NatureServe and Heritage Network Partners. \Sexpr{format(Sys.Date(), "%Y")}. Species distribution model for \Sexpr{as.character(ElementNames$CommName)} (\textit{\Sexpr{as.character(ElementNames$SciName)}}). Created on \Sexpr{format(Sys.Date(), "%d %b %Y")}. Arlington, VA with Network partners from VA, PA, and NY.
\end{minipage}
}
...
\begin{thebibliography}{99}\setlength{\itemsep}{-1pt}
  ...
  \bibitem{LiuEtAl2015} Liu, C., G. Newell, and M. White. 2015. On the selection of 
    ...
    Evolution 6:337–348. 
  \bibitem{SofaerEtAl2019} Sofaer, H. R., C. S. Jarnevich1, I. S. Pearse, R. L. Smyth, S. Auer, G. L. Cook, T. C. Edwards, Jr., G. F. Guala, T. G. Howard, J. T. Morisette, and H. Hamilton. 2019. The development and delivery of species distribution models to inform decision-making. BioScience.
...
\normalsize
\noindent
Appendix 1. Descriptions for environmental variables included in model. Variables were derived from the StreamCat {\protect\NoHyper\cite{streamcat}\protect\endNoHyper} and National Hydrography Dataset (NHD) Plus {\protect\NoHyper\cite{nhd}\protect\endNoHyper} datasets.
\begin{center}
\begin{table}[h]
\resizebox{\textwidth}{!}{
<<Appendix1, results="asis", echo = FALSE>>=
...
\normalsize
\noindent
Appendix 2. Model details for reproducibility. All R Scripts are available at \href{https://github.com/HeritageNetwork/Regional_SDM}{github}.
\begin{itemize}
    \setlength{\itemsep}{0pt}
    \setlength{\parskip}{0pt}
    \setlength{\parsep}{0pt}
